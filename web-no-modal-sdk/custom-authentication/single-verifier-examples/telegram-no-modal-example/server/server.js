...
const { TELEGRAM_BOT_NAME, TELEGRAM_BOT_TOKEN, SERVER_HOST_URL } = process.env;
...
app.get("/telegram/login", (req, res) => {
    ...
    fs.readFile(file, "utf8", (err, data) => {
        ...
        const result = data.replace(/{{TELEGRAM_BOT_NAME}}/g, TELEGRAM_BOT_NAME)
...
app.get("/telegram/callback", async (req, res) => {
    ...
    try {
        ...
        const redirectUrl = `http://localhost:3000?token=${JWTtoken}`; // Redirect back to frontend with token
