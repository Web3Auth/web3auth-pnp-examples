...
function App() {
  const [mfaLoginInterface, setMfaLoginInterface] =
    useState<UserLoginInterface | null>(null);
  const [nonMfaLoginInterface, setNonMfaLoginInterface] =
    useState<UserLoginInterface | null>(null);
  ...
  const login = async () => {
    try {
      ...
      if (hadEnabledMfa) {
        ...
        blockchainProvider = await mfaLoginInterface.loginWithWeb3Auth(
          idToken,
          loginRes.user.uid
        );
        setHasMfaEnabled(true);
        ...
        blockchainProvider = await nonMfaLoginInterface.loginWithWeb3Auth(
          idToken,
          loginRes.user.uid
        );
        setHasMfaEnabled(false);
      ...
      if (!blockchainProvider) {
        throw new Error(
          "Error while login, check console logs for more details"
        );
      }
  ...
  return (
    <div className="container">
      ...
      <footer className="footer">
        <a
          href="https://github.com/Web3Auth/examples/tree/master/web-core-sdk/one-key-flow/modular-example"
          target="_blank"
